...
class Parser(private val scanner: Scanner, private val errorHandler: ErrorHandler) {
    ...
    private fun makeInstruction(labels: MutableList<Token>, opcode: Token, arg: Token?)
        ...
        return when (opcode.symbol) {
            ...
            Symbol.BITAND -> InstructionBITAND(labels, opcode)
            Symbol.BITOR -> InstructionBITOR(labels, opcode)
            Symbol.BITXOR -> InstructionBITXOR(labels, opcode)
            Symbol.BITNOT -> InstructionBITNOT(labels, opcode)
            Symbol.SHL -> InstructionSHL(labels, opcode)
            Symbol.SHR -> InstructionSHR(labels, opcode)
     ...
     */
    private fun error(errorMsg: String): ParserException = ParserException(scanner.position, errorMsg)

    /**
     * Create a parser exception with the specified error position and message.
     */
    private fun error(errorPos: Position, errorMsg: String): ParserException = ParserException(errorPos, errorMsg)
}
